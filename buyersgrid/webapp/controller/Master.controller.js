sap.ui.define([
	...
	return BaseController.extend("cf.buyersgrid.controller.Master", {
		...
		onAfterRendering: function (oEvent) {
			...
			$("#container-buyersgrid---master--list").on('scroll', {
				g: this
			}, this.loadOnScroll);
			...
			// this.selectedWH = document.getElementById('container-buyersgrid---App--wh-labelText').innerHTML;
		...
		onFilter: function () {
			...
			if (!this.byId("filter")) {
				Fragment.load({
					...
					name: "cf.buyersgrid.view.Filter",
					controller: this
		...
		fetchProducts: function () {
			...
			if (this.selectedChannel !== "" && this.selectedChannel !== null && this.selectedChannel !== undefined && this.selectedCat !==
				...
				/*if (this.globalSearchField !== "" && this.globalSearchField !== undefined && this.globalSearchValue !== "" && this.globalSearchValue !==
					undefined) {*/
				/*var url = "pricing/PriceGrid.xsodata/ProductView?$filter=GRID_TYPE__C eq 'BGP' and SALESFORCE_PRODUCT_CATEGORY__C eq '" + this.selectedCat +
				...
				/*var url = "pricing/Productview.xsjs?globalsearchKey=" + this.globalSearchValue +
						...
						"&$filter=GRID_TYPE__C eq 'BGP' and SALESFORCE_PRODUCT_CATEGORY__C eq '" + this.selectedCat +
						"' and WAREHOUSE_CODE__C eq '" + this.selectedWH + "' " + urlBrand + ")&$format=json";	
				} */
				if (this.globalSearchValue !== "" && this.globalSearchValue !== undefined) {
					var url = "pricing/Productview.xsjs?Whcode=" + this.selectedWH + "&Gridtype=BGP&Productcategory=" + this.selectedCat +
		...
		onGlobalSearch: function (oEvent) {
			...
			// var url = "pricing/Productview.xsjs?globalsearchKey=" + sQuery + "&limit=100&from=1&$format=json";
			var url = "pricing/GBSProductview.xsjs?globalsearchKey=" + sQuery + "&Gridtype=BGP&$format=json";
		...
		onSortDialogForPhone: function (oEvent) {
			...
			if (!this.sortDialog) {
				this.sortDialog = sap.ui.xmlfragment("cf.buyersgrid.view.ViewSettingsDialog", this);
				...
				if (width < 700) {
					var selected = priceData.filter(function (a) {
						...
						/*return a[h0] === oItem.getNumber() && a[h1] === oItem.getTitle() && a[h2] === oItem
							.getIntro() && a[h3] === oItem.getNumberUnit();	*/ //commented for the issue about data not loading in detail page. 
					});
